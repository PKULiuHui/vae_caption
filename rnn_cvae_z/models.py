...
class AttnDecoder(nn.Module):
    ...
    def __init__(self, embed, attention_dim, embed_dim, decoder_dim, latent_size, vocab_size, encoder_dim=2048, dropout=0.5):
        """
        ...
        self.init_h = nn.Linear(encoder_dim + decoder_dim * 2, decoder_dim)  # linear layer to find initial hidden state of LSTMCell
        self.init_c = nn.Linear(encoder_dim + decoder_dim * 2, decoder_dim)  # linear layer to find initial cell state of LSTMCell
...
class CvaeEncoder(nn.Module):
    ...
    def __init__(self, standard_gaussian, hidden_size, img_size, latent_size):
        ...
        self.fc1 = nn.Linear(hidden_size * 2 + img_size, hidden_size)
        self.hidden2mu1 = nn.Linear(hidden_size, latent_size)
        self.hidden2logv1 = nn.Linear(hidden_size, latent_size)
        self.rec = nn.Linear(latent_size, hidden_size * 2 + img_size)
...
class ImgCapCVAE(nn.Module):
    ...
    def __init__(self, standard_gaussian, attn_dim, embed_dim, hidden_size, latent_size, vocab_size, img_size=2048):
        ...
        self.attnDecoder = AttnDecoder(self.embedding, attn_dim, embed_dim, hidden_size, latent_size, vocab_size)
    ...
    def forward(self, imgs, encoded_captions, caption_lengths, val=False):
        ...
        return preds, alphas, mu1, logv1, img_text, img_text_rec, mu2, logv2, encoded_captions, decode_lengths, sort_ind
