...
save_folder = 'ckpt1/'  # save folder path, for saving different experiment results
...
standard_gaussian = True  # use standard gaussian (one distribution) or not (two distributions)
...
def main():
    ...
    cvae_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, model.cvaeEncoder.parameters()), lr=lr)
    optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, chain(model.imgEncoder.parameters(),
                                                                                model.capEncoder.parameters(),
                                                                                model.attnDecoder.parameters())), lr=lr)
...
def loss_fn(preds, target, alphas, mu1, logv1, it, it_rec, mu2, logv2, step):
    # Reconstruction loss
    criterion = torch.nn.CrossEntropyLoss(reduction='mean', ignore_index=0)
    nll_loss = criterion(preds, target)
    ...
    rec_loss = criterion_rec(it_rec, it)

    # kl_w = kl_anneal_function(step)
    kl_w = 1

    return nll_loss, attn_loss, kl_loss, kl_w, rec_loss


def train(train_loader, model, cvae_optimizer, optimizer, epoch):
    """
    Performs one epoch's training.

    :param train_loader: DataLoader for training data
    :param model: CVAE model
    :param cvae_optimizer: CVAE encoder optimizer
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # Forward prop.
        scores, alphas, mu1, logv1, it, it_rec, mu2, logv2, caps_sorted, decode_lengths, sort_ind = model(imgs, caps, caplens)
        ...
        nll_loss, attn_loss, kl_loss, kl_w, rec_loss = loss_fn(scores, targets, alphas, mu1, logv1, it, it_rec, mu2, logv2, step)

        cvae_loss = kl_w * kl_loss + rec_loss
        cvae_optimizer.zero_grad()
        cvae_loss.backward(retain_graph=True)

        loss = nll_loss + alpha_c * attn_loss
        optimizer.zero_grad()
        loss.backward()
        ...
        nll_losses.update(nll_loss.item(), sum(decode_lengths))
...
def validate(val_loader, model, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            step = float(epoch + 1)
            nll_loss, attn_loss, kl_loss, kl_w, rec_loss = loss_fn(scores, targets, alphas, mu1, logv1, it, it_rec, mu2, logv2, step)
            loss = nll_loss + alpha_c * attn_loss + kl_w * kl_loss + rec_loss
