...
# CVAE parameters
latent_size       = 512  # gaussian distribution size
...
def main():
    ...
    optimizer_recon = WarmUpOpt(optimizer=torch.optim.Adam(chain(model.imgEncoder.parameters(),
                                                            model.capEncoder.parameters(),
                                                            model.attnDecoder.parameters()),
                                               lr=lr,
                                               betas=(0.9, 0.99),
                                               eps=1e-9,
                                               weight_decay=0),
                          ...
                          warmup_steps=4000,
                          factor=1)
    optimizer_kl = WarmUpOpt(optimizer=torch.optim.Adam(model.cvaeEncoder.parameters(),
                                               lr=lr,
                                               betas=(0.9, 0.99),
                                               eps=1e-9,
                                               weight_decay=0),
...
def train(train_loader, model, optimizer_recon, optimizer_kl, epoch):
    """
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        loss = rec_loss + alpha_c * attn_loss
        ...
        if grad_clip is not None:
            ...
            torch.nn.utils.clip_grad_norm_(chain(model.imgEncoder.parameters(),
                                                 model.capEncoder.parameters(),
                                                 model.attnDecoder.parameters()), 5)

        # Update weights
        optimizer_recon.step(zero_grad=True)
        loss = kl_w * kl_loss
        loss.backward()
        if grad_clip is not None:
            # clip_gradient(optimizer, grad_clip)
            torch.nn.utils.clip_grad_norm_(model.cvaeEncoder.parameters(), 5)
        optimizer_kl.step()
...
def validate(val_loader, model, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            # Forward prop.
            scores, alphas, mu1, logv1, mu2, logv2, caps_sorted, decode_lengths, sort_ind = model(imgs, caps, caplens, val=True)
