...
class EncoderCell(nn.Module):
    ...
    def forward(self, inputs, PadMask):
        outputs, _ = self.attn(inputs, inputs, inputs, PadMask)
        inputs = self.layerNorm(inputs + outputs)
...
class DecoderCell(nn.Module):
    ...
    def forward(self, inputs, encoder_outputs, PadMask, SeqMask):
        outputs, _ = self.maskedAttn(inputs, inputs, inputs, SeqMask)
        inputs = self.layerNorm(inputs + outputs)
        outputs, weight = self.attn(inputs, encoder_outputs, encoder_outputs, PadMask)
        inputs = self.layerNorm(inputs + outputs)
        return self.layerNorm(inputs + self.dropout(self.FFN(inputs))), weight
